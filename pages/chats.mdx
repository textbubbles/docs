...
| `participants` | string[] \| object[] | **iMessage:** array of phone/Apple ID strings. **WhatsApp:** array of `{ handle, isPhoneResolved, admin }` — see [WhatsApp Groups](/whatsapp/groups#participant-fields) |
...
| `participants` | string[] \| object[] | Current roster. **iMessage:** plain strings. **WhatsApp:** `{ handle, isPhoneResolved, admin }` — see [Participants](#participants) below |
