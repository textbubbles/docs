...
Set or replace the group chat icon. Group chats only — 1:1 chats have no icon. Works on both iMessage and WhatsApp.

### Request

Supply exactly one of `imageUrl` (a publicly downloadable URL) or `imageBase64` (raw base64-encoded bytes).
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `imageUrl` | string | One of | Publicly reachable URL of the new icon. The API fetches it server-side |
...
`photoUrl` is a signed URL to the persisted copy of the new icon. It can be `null` when the change reached the provider but the persisted copy could not be captured (e.g. a transient storage failure) — the icon change still goes through and the `chat.photo.changed` webhook still fires. Call `GET /v1/chats/:chatId` later to retrieve a fresh signed URL.

Emits `chat.photo.changed`.

## DELETE /v1/chats/:chatId/icon — Remove the group chat icon

Remove the group chat icon. Group chats only. Works on both iMessage and WhatsApp.
