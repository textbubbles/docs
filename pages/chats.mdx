...
> **Android (SMS) groups — coming soon (WIP).** Today, iMessage group chats require every participant to be on iMessage. Adding an Android (SMS) recipient is not yet supported; it's on the roadmap.
...
| `channel` | string | No | `"imessage"` (default) or `"whatsapp"`. Android (SMS) coming soon |
