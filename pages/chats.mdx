...
>
> **`chatId` means two related things.** In a path parameter (`/v1/chats/:chatId/...`) and on `chat.*` webhook events it's the **provider chat identifier** — `iMessage;+;<guid>` for iMessage, `<id>@g.us` for WhatsApp groups. In a response body's `chat` fragment it's the **stable UUID** of the persisted conversation row; the provider identifier is exposed as `providerChatGuid` alongside it. The API accepts either form as the `:chatId` path parameter, and the UUID is the value to use for sends keyed to an existing chat via `POST /v1/messages`.
>
...
| `chat.chatId` | string | UUID of the persisted conversation row. Stable across channels. Use this as the `conversationId` for sends via `POST /v1/messages` |
...
- The response's `chat.providerChatGuid` is the WhatsApp group JID (`<id>@g.us`). Use it as the `:chatId` path parameter on the other `/v1/chats/*` endpoints. The `chat.chatId` UUID is the stable identifier for sends via `POST /v1/messages`.
