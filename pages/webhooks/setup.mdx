...
| `secret` | no | Your own HMAC-SHA256 signing secret. If omitted, a random one is generated and returned once. See [Signing secret](#signing-secret) below |

### Signing secret

Every webhook delivery is signed with HMAC-SHA256 using a secret that's unique to that webhook. You verify the signature on your endpoint to prove the request came from TextBubbles and wasn't tampered with in transit (see [Signature Verification](#signature-verification)).

You can either let the API generate the secret for you or supply your own.

**Option 1 — let the API generate one (recommended).** Omit `secret` from the create request. The API returns a fresh, cryptographically random secret in the `secret` field of the response, prefixed with `whsec_`. This is the example shown above.

> **The `secret` field is returned only on creation, and only when you did not supply one.** Store it immediately in your secrets manager — it is never returned again by any other endpoint. If you lose it, use [Rotate Signing Secret](#rotate-signing-secret) to mint a new one.

**Option 2 — supply your own.** Useful when you already manage signing keys in your own secrets system (e.g. AWS Secrets Manager, Vault, Doppler) and want the same value on both sides without an extra copy step. Pass `secret` in the request body:
...
When you supply your own `secret`, the API stores it but does **not** echo it back in the response (or in any subsequent `GET` / `list` response). You are responsible for keeping your copy.

**Choosing a self-supplied secret:**

- Use a high-entropy random value — at least 32 bytes (e.g. `openssl rand -hex 32`). Do not use a human-chosen passphrase.
- Treat it as opaque bytes. The `whsec_` prefix is a convention to make leaked secrets easier to spot in logs and source control, but the API does not require it.
- Never commit it to source control. Never reuse it across webhooks — each webhook gets its own.
- Rotate immediately via [Rotate Signing Secret](#rotate-signing-secret) if you suspect it has leaked. Rotation invalidates the old secret instantly.
...
Your endpoint must respond with a `2xx` status code within 10 seconds. Anything else is classified as either **retryable** or **terminal**:

| Response | Treated as | Behavior |
|---|---|---|
| `2xx` | Success | Delivery recorded as `delivered`. Resets the consecutive-failure streak. |
| `408` Request Timeout, `429` Too Many Requests | Retryable | Retried with exponential backoff (see table below). Counts toward auto-deactivation if all retries fail. |
| Any other `4xx` (`400`, `401`, `403`, `404`, `410`, `422`, …) | **Terminal** | Recorded once as `failed`. **Not retried.** Does not count toward auto-deactivation. The event is dropped from this webhook's queue. |
| `5xx`, network errors (timeout, DNS, TCP reset) | Retryable | Same as 408/429. |

To acknowledge an event you intentionally skip (e.g. unknown resource, ignored event type), return `200` with a body like `{"success": true, "message": "ignored"}`. Returning `4xx` will drop the event from this webhook's queue.

**Retryable retry schedule (exponential backoff):**
...
**Auto-deactivation:** When three consecutive deliveries to the same webhook each exhaust the full retry chain, the webhook is automatically deactivated (`active=false`). Any `2xx` delivery resets the counter. Terminal 4xx responses do not contribute to the counter.
