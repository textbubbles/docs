...
Your endpoint must respond with a `2xx` status code within 30 seconds. Anything else is classified as either **retryable** or **terminal**:
...
This matches the convention every major webhook provider uses (Stripe, GitHub, Twilio, Shopify, Slack): a `4xx` from your endpoint means "I won't process this, don't retry," so we don't. Use `200` with an "ignored" body when you want to acknowledge an event you've intentionally chosen to skip.
...
**Auto-deactivation:** When three consecutive deliveries to the same webhook all exhaust their retry chain, the webhook is automatically deactivated (`active=false`). Any single `2xx` delivery resets the counter. Terminal 4xx responses do **not** contribute to the counter — they're recorded as failed and dropped, but the webhook stays active so unrelated events continue to deliver.
