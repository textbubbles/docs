...
| `errorCode` | Provider-level error code (present on `message.failed` only). For iMessage this is `imessage_error_N` where `N` is Apple's raw error flag. The most common value is `imessage_error_4` ("Not Delivered" — Apple declined to deliver, usually because the recipient's device was unreachable for an extended period or because Apple flagged the sender account). Absent when the failure was detected without an upstream code. |
