...
| Field | Description |
|-------|-------------|
| `reaction.type` | Reaction type. **iMessage:** `love`, `like`, `dislike`, `laugh`, `emphasize`, `question`, `emoji`, `remove`. **WhatsApp:** the raw emoji character (e.g. `❤️`, `🔥`), or `remove` when the reaction was cleared. The `remove` spelling is shared across both channels — a single string covers a cleared reaction regardless of channel. |
