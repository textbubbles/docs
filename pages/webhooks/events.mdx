...
| `chat.photo.changed` | A group chat's photo was updated (iMessage only) |

> **Single-emit dedup window.** When a customer initiates a rename / add / remove via `POST /v1/chats/:chatId/name`, `POST /v1/chats/:chatId/participants`, or `DELETE /v1/chats/:chatId/participants/:participantId` **and** the upstream provider also fires its own state-change event, the corresponding webhook (`chat.title.changed` / `chat.participant.added` / `chat.participant.left`) is emitted exactly once — duplicates within a 60-second window are suppressed. Implement your handler to be idempotent on `eventId` either way.
