...
| `error` | Failure reason. Present on `message.failed` only. **Always a plain string** — either a short error code (e.g. `NO_CHANNEL_AVAILABLE`) or a human-readable message from the upstream provider. Not an object; do not read `error.message` |
...
`externalMessageId` is omitted when the failure occurs before the message reached the upstream provider (e.g. routing failures). `error` is always a string — log it directly rather than treating it as a structured object.
...
### WhatsApp group inbound

WhatsApp messages received from a group chat are currently emitted as `message.inbound` events. The `from` field is set to the **group's identifier expressed as an E.164-shaped string** (the platform synthesizes one from the group's stable internal ID), not the individual sender's number — so a single value identifies the group, but it is **not** a real mobile number and will not match a contact's E.164.

The webhook payload does not currently include a dedicated `isGroup` flag or `chat` object. If you do not want to surface group threads in your inbox, the recommended workaround is to maintain an allowlist of expected fan E.164 numbers and drop inbounds whose `from` is not on the list; group-emitted values will fall outside that set. A first-class group-inbound shape is on the roadmap.

> Sending **into** an existing WhatsApp group is fully supported and uses the JID form `to: "...@g.us"` — see [WhatsApp → Sending](/whatsapp#sending). The note above is specifically about the **inbound** webhook shape.
