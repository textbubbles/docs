...
| `attachments[].downloadUrl` | string | Pre-signed URL to download the attachment. Time-limited (1 hour) and self-authenticated — no API key required. Fetch directly via GET request. Same shape across all channels — your inbox code does not need to branch on `channel` to fetch media. |
| `attachments[].note` | string \| undefined | Present only on rare failure cases when the attachment metadata was received but the bytes couldn't be persisted (e.g. transient upstream issue at receive time). When present, `downloadUrl` will be absent. |
