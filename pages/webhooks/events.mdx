...
| Field | Description |
|-------|-------------|
| `externalMessageId` | BlueBubbles GUID of the Keep system event itself |
| `targetMessageId` | Internal message ID of the audio message that was kept. `null` if the audio predates our retention or was sent through a different BlueBubbles instance — `targetExternalId` is still populated so you can cross-reference if you store BB GUIDs |
| `targetExternalId` | BlueBubbles GUID of the audio message that was kept |
| `from` | Contact who tapped Keep |
| `to` | Instance phone number the audio was sent from |

## Chat Event Payloads

All four chat events share the same envelope. Each carries `externalMessageId` (the BlueBubbles GUID of the chat.db system row), `chatId` (BlueBubbles chat identifier — for 1:1 chats this is the contact address; for group chats it's an opaque identifier you can match against the `chat` resource), and `actor` (who performed the action).
...
| Field | Description |
|-------|-------------|
| `chatId` | BlueBubbles chat identifier |
