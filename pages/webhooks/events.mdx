...
| `chat.photo.changed` | A group chat's photo was updated (iMessage or WhatsApp) |

> **Single-emit dedup window.** When a customer initiates a rename / add / remove / icon change via `POST /v1/chats/:chatId/name`, `POST /v1/chats/:chatId/participants`, `DELETE /v1/chats/:chatId/participants/:participantId`, `POST /v1/chats/:chatId/icon`, or `DELETE /v1/chats/:chatId/icon` **and** the upstream provider also fires its own state-change event, the corresponding webhook (`chat.title.changed` / `chat.participant.added` / `chat.participant.left` / `chat.photo.changed`) is emitted exactly once — duplicates within a 60-second window are suppressed. Implement your handler to be idempotent on `eventId` either way.
...
| `conversation` | Optional object — see [Conversation and sender fragments](#conversation-and-sender-fragments). Present when the originating outbound message was associated with a conversation |
| `sender` | Optional object — see [Conversation and sender fragments](#conversation-and-sender-fragments) |
...
| `conversation` | object \| absent | Present when the inbound was attributed to a known chat. See [Conversation and sender fragments](#conversation-and-sender-fragments) |
| `sender` | object \| absent | Present alongside `conversation`. Resolves the sender to a contact when one matches. See [Conversation and sender fragments](#conversation-and-sender-fragments) |
...
### Conversation and sender fragments

`message.inbound` and outbound lifecycle events (`message.sent`, `message.delivered`, `message.read`, `message.failed`) include `conversation` and `sender` objects when the message is attributed to a known chat. Top-level `from`/`to` remain populated for backwards compatibility — consumers that only read those keys continue to work.
...
```json
{
  ...
  "data": {
    ...
    "conversation": {
      ...
      "participants": [
        { "handle": "+14155551234", "contactId": "9e1c8f63-1c7d-4a4b-b9a3-2f31b3a72e10", "role": "member" },
        { "handle": "+14155559876", "contactId": null, "role": "member" }
      ]
    },
    "sender": {
      "contactId": "9e1c8f63-1c7d-4a4b-b9a3-2f31b3a72e10",
...
| Field | Type | Description |
|-------|------|-------------|
| `id` | string | Stable UUID for the conversation. Use this in any future chat-keyed call |
| `isGroup` | boolean | `true` for group chats, `false` for 1:1 |
| `providerChatGuid` | string \| null | Provider chat identifier (the same `chatId` returned by `POST /v1/chats/groups` and surfaced on `chat.*` events) |
| `channel` | string | `imessage`, `sms`, or `whatsapp` |
| `groupName` | string \| null \| absent | Group display name. Present only when `isGroup` is `true`; null if the group has no name set |
| `participants` | array \| absent | Active roster. Present only when `isGroup` is `true`. Each entry carries `handle`, `contactId`, `role`, and — on WhatsApp groups — `isPhoneResolved`. `role` is `member`, `owner`, or `self` (the customer's own instance). `isPhoneResolved` is `true` when `handle` is a phone-based identifier and `false` when only a Linked Device ID is known; treat this as the source of truth on whether the phone is resolved, rather than parsing `handle` |
...
Outbound lifecycle events (`message.sent`, `message.delivered`, `message.read`, `message.failed`) include the same `conversation` and `sender` objects when the originating outbound message was associated with a conversation. Status events for legacy outbound messages without a stored conversation omit both fields.
...
| Field | Description |
|-------|-------------|
| `chatId` | Provider chat identifier (`iMessage;+;<guid>` or `<id>@g.us`) |
| `participant` | Contact added to (or removed from) the chat. On WhatsApp, a JID; may be `<id>@lid` when the phone is not yet resolved |
...
| `photoUrl` | string \| null \| undefined | Signed URL to the new chat photo, `null` when the photo was removed, or omitted if the photo could not be captured (e.g. provider unreachable). The event always fires |
