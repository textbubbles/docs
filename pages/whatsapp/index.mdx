...
# Wipe stored credentials but keep the number registered — use before re-pairing
curl -X POST -H "Authorization: Bearer $TB_KEY" \
  "https://api.textbubbles.com/v1/whatsapp/numbers/$NUM/reset-auth"
```

Call `reset-auth` before re-pairing a previously linked number: a fresh QR link must not reuse the old session's credentials. After it returns, run the [pairing flow](#pairing-a-number) again with `enable`.
...
`WHATSAPP_RATE_LIMITED` and `WHATSAPP_REJECTED` can arrive after a `message.sent` event — WhatsApp acknowledges relay first and can reject afterwards. Treat a subsequent `message.failed` as authoritative.
...
| `WHATSAPP_RATE_LIMITED` | WhatsApp temporarily rate-limited the sending account for new outreach (provider error 463). Not retryable until the limit expires (typically hours). Messages to contacts with an existing conversation are usually unaffected. |
| `WHATSAPP_REJECTED` | WhatsApp rejected the message after relay. The raw provider code is included in the error message. |
