...
- **Contacts** — contacts are isolated per customer
...
API keys are **hashed with bcrypt** before storage. The plaintext key is returned **only once** at creation time and cannot be retrieved later. If you lose your key, you must generate a new one via the admin API.
