...
To send an audio file as a native iMessage voice-memo bubble (inline player with waveform) instead of a generic file attachment, set `isAudioMessage: true` on the attachment. Voice memos are attachment-only — omit `content.text`.

Accepted formats: any audio (MP3, Opus/AAC/PCM in CAF, webm, m4a, ogg, wav, …). The server normalizes non-compatible inputs to Opus-in-CAF — the format iMessage's voice-memo UI can read duration from. MP3 and Opus-in-CAF are forwarded as-is. If you can, encode client-side to MP3 or Opus-in-CAF to skip the extra server transcode.
...
| `attachments[].isAudioMessage` | boolean | No | Render as a native iMessage voice-memo bubble (inline player). Accepts any audio format — the server transcodes non-compatible inputs (anything except MP3 or Opus-in-CAF) to Opus-in-CAF. iMessage only. |
