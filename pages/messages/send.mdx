...
To send into an existing chat (group or 1:1), pass `chatId` instead of `to`. The `chatId` value is the `chat.chatId` UUID returned from [`POST /v1/chats/groups`](/chats#post-v1chatsgroups--create-a-group-chat) or [`GET /v1/chats/:chatId`](/chats#get-v1chatschatid--get-chat-info).

- `to` and `chatId` are mutually exclusive — provide exactly one. Supplying both, or neither, returns `400 INVALID_REQUEST`.
- `conversationId` is accepted as an alias for `chatId` — either field name routes identically. Supply only one; supplying both with different values returns `400 INVALID_REQUEST`.
...
When the send is keyed by `chatId`, the 202 response carries `conversationId` (the same UUID) in place of `to`. The matching `message.*` lifecycle webhooks for this send carry the full `chat` fragment — see [Chat and sender fragments](/webhooks/events#chat-and-sender-fragments).
...
| WhatsApp | MP4 with H.264 video and AAC audio |
...
| `conversationId` | string (UUID) | No | Alias for `chatId` — either field name routes identically. Supply only one; supplying both with different values returns `400 INVALID_REQUEST` |
