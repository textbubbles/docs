...
Re-send a message that previously failed (for example, because the delivering Mac mini was offline when the original attempt ran). Only messages currently in `status: failed` can be retried — messages that are still `queued`, `sent`, or in any other state return `400 INVALID_STATUS`.
