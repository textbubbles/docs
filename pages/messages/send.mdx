...
Send iMessages with automatic SMS fallback via `POST /v1/messages`. All messaging (iMessage, SMS, MMS, RCS) is handled natively through BlueBubbles — no external providers required.
...
By default, messages try iMessage first and fall back to SMS. Both channels are handled natively by BlueBubbles. Customize with the `routing` field:
