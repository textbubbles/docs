...
|-----|-------|----------|--------------|-------|
| Send a file (photo, video, PDF, audio, any MIME type) | `attachments[]` | iMessage and SMS/MMS | `type: "url"` (HTTPS) or `type: "base64"` | Required for non-image files. On SMS, each attachment is delivered as a separate MMS; carrier delivery of non-image MIME types is not guaranteed. |
...
1. **Do you need to send a non-image file** (PDF, video, audio, vCard, etc.)? Use `attachments[]` with `type: "url"` or `type: "base64"`. SMS/MMS cannot carry arbitrary file types in all carrier networks — [check capabilities](/api-reference#get-v1capabilitiesphonenumber) first.
2. **Do you want to send 2+ images as a carousel** (single grouped iMessage bubble)? Use `content.mediaUrls`. Do not use `attachments[]` for this — only `mediaUrls` triggers carousel messageType.
3. **Do you have raw bytes** (not a public URL)? Use `attachments[]` with `type: "base64"`.
4. **Everything else (single image with an HTTPS URL):** either works; prefer `attachments[]` for explicit control over `mimeType` and `filename`.
