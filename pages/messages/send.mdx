...
To send an audio file as a native iMessage voice-memo bubble (inline player with waveform) instead of a generic file attachment, set `isAudioMessage: true` on the attachment. The audio must be in CAF format (or MP3 — BlueBubbles converts MP3 to CAF server-side). Voice memos are attachment-only — omit `content.text`.
...
| `attachments[].isAudioMessage` | boolean | No | Render as a native iMessage voice-memo bubble (inline player). Requires CAF audio (or MP3 — BlueBubbles converts it). iMessage only. |
