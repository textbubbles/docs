...
- `preference` — ordered list of channels: `"imessage"`, `"sms"`, `"whatsapp"`. When you supply this field explicitly, channels are attempted **strictly in the order given**: for each channel that requires a reachability probe (iMessage, WhatsApp), the recipient is checked live, and only if the probe says the recipient is unreachable on that channel does the next channel get tried. Omitting `preference` uses the default `["imessage", "sms"]` along with a short-lived availability cache — slightly faster, but a recently-cached "not on iMessage" answer can carry across briefly. Specify `preference` explicitly when you need the strict-order guarantee.
