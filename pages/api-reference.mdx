...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/v1/webhooks` | [Get webhook config](/webhooks/setup#get-current-configuration) |
| `PUT` | `/v1/webhooks` | [Set webhook URL & events](/webhooks/setup) |
| `DELETE` | `/v1/webhooks` | [Delete webhook config](/webhooks/setup#delete-webhook-configuration) |
| `POST` | `/v1/webhooks/test` | [Send a test event to your webhook](/webhooks/setup#send-a-test-event) |
| `POST` | `/v1/webhooks/rotate-secret` | [Rotate signing secret](/webhooks/setup#rotate-signing-secret) |
...
SMS fallback is handled natively by iMessage on the Mac mini — when an iMessage send fails to a non-iMessage recipient, Messages.app automatically falls back to SMS via the connected iPhone. There is no separate SMS provider to monitor.
