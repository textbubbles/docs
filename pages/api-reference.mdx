...
| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/v1/chats/groups` | [Create a group chat](/chats#post-v1chatsgroups--create-a-group-chat) |
| `GET` | `/v1/chats/:chatId` | [Get chat info](/chats#get-v1chatschatid--get-chat-info) |
| `PUT` | `/v1/chats/:chatId/name` | [Rename a group chat](/chats#put-v1chatschatidname--rename-a-group-chat) |
| `POST` | `/v1/chats/:chatId/participants` | [Add a participant](/chats#post-v1chatschatidparticipants--add-a-participant) |
| `DELETE` | `/v1/chats/:chatId/participants/:participantId` | [Remove a participant](/chats#delete-v1chatschatidparticipantsparticipantid--remove-a-participant) |
| `POST` | `/v1/chats/:chatId/leave` | [Leave a group chat](/chats#post-v1chatschatidleave--leave-a-group-chat) |
| `POST` | `/v1/chats/:chatId/read` | [Mark a chat as read](/chats#post-v1chatschatidread--mark-a-chat-as-read) |
| `POST` | `/v1/chats/:chatId/unread` | [Mark a chat as unread](/chats#post-v1chatschatidunread--mark-a-chat-as-unread) |
| `POST` | `/v1/chats/:chatId/typing` | [Send a typing indicator](/chats#post-v1chatschatidtyping--send-a-typing-indicator) |
...
| `NAME_REQUIRED` | 400 | `POST /v1/chats/groups` with `channel: "whatsapp"` and no `name` |
| `NOT_FOUND` | 404 | Resource not found |
| `REPLY_NOT_FOUND` | 404 | Reply target message not found |
| `CHANNEL_NOT_SUPPORTED` | 400 | Operation not supported on the conversation's channel (e.g. `/v1/chats/:chatId/read` or `/unread` on a WhatsApp chat) |
| `NO_CHANNEL_AVAILABLE` | 400 | Recipient not reachable |
| `FROM_INSTANCE_CANNOT_REACH` | 400 | The specified `from` address cannot reach the recipient on any of the requested channels, or — for `chatId` sends — does not match the instance the chat is pinned to |
| `INVALID_REQUEST` | 400 | Request shape is invalid. On `POST /v1/messages`: `to` and `chatId` were both supplied, or neither was supplied (exactly one is required) |
| `CONVERSATION_NOT_FOUND` | 404 | `chatId` does not match any conversation for this customer |
...
| `RATE_LIMIT_EXCEEDED` | 429 | Too many requests on this API key. Inspect `X-RateLimit-Limit` / `X-RateLimit-Remaining` / `Retry-After` on the response. Per-key budget covers all endpoints (sends + status polls + listings). |
| `RATE_LIMIT_PHONE_EXCEEDED` | 429 | Too many sends from a single `from` phone number. Independent of `RATE_LIMIT_EXCEEDED`. Inspect `X-RateLimit-Limit-Phone` / `X-RateLimit-Remaining-Phone` / `Retry-After`. Distinct so you can tell "my key's overall traffic is over budget" from "this specific line is being driven too hard". |
