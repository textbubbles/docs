...
**Known outstanding leaks** (tracked here so the next pass catches them; do not let new PRs add more): `pages/api-reference.mdx` (health example + monitoring section + `BLUEBUBBLES_TIMEOUT` error code + Mac mini SMS-fallback sentence), `pages/webhooks/events.mdx` (`BlueBubbles GUID` and `BlueBubbles chat identifier` field descriptions), `pages/messages/reactions.mdx` (`BlueBubbles GUID`), and the mirrored sections in `public/llms-full.txt` + `public/llms.txt`. The live API still echoes `BLUEBUBBLES_TIMEOUT` (error code) and `"bluebubbles"` (health-check key) — renaming those is a breaking change for customers that needs a coordinated `api/` + `docs/` release, not a silent doc swap.
