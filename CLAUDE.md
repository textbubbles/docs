...
**Never describe failure modes, latency, or behavior in terms of our physical or hosting infrastructure.** Specifically, do not reference:
...
**What you CAN mention** (already public throughout the docs, do not strip retroactively without asking):

- **BlueBubbles** as the iMessage provider — appears in webhook field docs (`externalMessageId = BlueBubbles GUID`), health check response shape, and error codes (`BLUEBUBBLES_TIMEOUT`). Customers need these identifiers to reconcile data, so they are load-bearing.
...
If you're unsure whether a detail is public: **grep existing `pages/` and `public/llms*.txt` for it.** If it's already there and load-bearing, keep it. If not, default to generic language.
